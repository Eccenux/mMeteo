<?php
$inc_root = dirname(__FILE__);
require_once $inc_root."/SimpleJSLoader.php";
require_once $inc_root."/SimpleCSSLoader.php";
require_once $inc_root."/MultiFootersParser.php";
...
class ProcessingHelper
{
	...
	public function bottomFinalization($fileName)
	{
		if ($this->mode == 'dynamic') {
			...
			// append footers
			$p = new MultiFootersParser($content);
			$parsed_content = $p->parseDocument();
			// rewrite links from HTML to PHP
			echo preg_replace('#'.$fileName.'\\.html#i', $fileName.'.php', $parsed_content);
		}
	}
